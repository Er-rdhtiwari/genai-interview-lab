...
class LLMClient:
    ...
    def _call_oss_model_service(self, prompt: str) -> LLMGenerationResult:
        ...
        try:
            ...
            logger.warning(
                "OSS model service call failed (%s). Using local fallback response.",
                exc,
            )
            text, model = self._oss_local_fallback(prompt)
            return LLMGenerationResult(
                text=text,
                provider="oss",
                model=model,
    ...
    def _oss_local_fallback(self, prompt: str) -> tuple[str, str]:
        """
        Produce a deterministic, human-friendly fallback when the OSS service is unavailable.

        Tries to tailor a greeting if the prompt contains a Name: and birthday hints,
        otherwise returns a concise release note style summary.
        """
        ...
        if "birthday" in prompt.lower():
            text = (
                f"Happy birthday month, {name}! Wishing you a fantastic year ahead filled "
                "with good surprises. Enjoy your celebrations and take time for yourself."
            )
        else:
            snippet = prompt.strip().splitlines()[0][:160]
            text = (
                f"Release note summary: {snippet} "
                "(generated by OSS fallback while model service is offline)."
            )

        return text, "oss-fallback"
